...
vitis_hls_2023_2_path = '/tools/Xilinx/Vitis_HLS/2023.2'
...
config['Model']['Strategy'] = 'Resource' # Switch back to resource strategy in order to try and create routable design
config['Model']['ReuseFactor'] = 8     # Global Default RF
config['Model']['Precision'] = 'ap_fixed<16,6>' # Standard fixed point starting place
# 2.1 FORCE output layer to 32-bit so the DMA buffer maps cleanly to Python's np.int32
config['LayerName']['output_1']['Precision']['result'] = 'ap_fixed<32,16>'
...
#  Dense_1:
config['LayerName']['dense_1']['ReuseFactor'] = 10

# Dense_2:
config['LayerName']['dense_2']['ReuseFactor'] = 10

# Dense_3:
config['LayerName']['dense_3']['ReuseFactor'] = 10
#   -------                                                          -------------
...
hls_model = hls4ml.converters.convert_from_keras_model(
    model,
    hls_config=config,
    output_dir='my_vitis_project_RF10_IOSTREAM_PYTHON', # new dir for Vitis / Alveo approach
    backend='Vitis',  # Target Vitis backend instead of Vivado
    part='xcu280-fsvh2892-2l-e',    #Alveo U280 PL part num
    io_type='io_stream',     # New change: try using AXI-Master instead of default AXI-Lite
    clock_period=5.525       # Target 181MHz clock from previous RF 10 run
)
...
print(f"Calling hls_model.build...")
hls_model.build(
    csim=True,       # Skip C-simulation (already done in .compile)
    ...
    # bitstream=True   # Link the kernel to the U280 shell to create .xclbin
)
print(f"Called hls_model.build")

# 8. View the Reports
# This will show you exactly how many BRAMs/DSPs you used and the latency
hls4ml.report.read_vivado_report('my_vitis_project_RF10_IOSTREAM_PYTHON')